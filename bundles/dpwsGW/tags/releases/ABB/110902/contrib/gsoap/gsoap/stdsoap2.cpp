...
#ifdef __cplusplus
SOAP_SOURCE_STAMP("@(#) stdsoap2.cpp ver 2.7.14 2009-07-28 00:00:00 GMT")
extern "C" {
#else
SOAP_SOURCE_STAMP("@(#) stdsoap2.c ver 2.7.14 2009-07-28 00:00:00 GMT")
#endif
  ...
  if (!soap_tag_cmp(endpoint, "https:*"))
  {
    ...
    { if ((r = SSL_connect(soap->ssl)) <= 0)
      { int err = SSL_get_error(soap->ssl, r);
        ...
        { register int s;
          if (err == SSL_ERROR_WANT_READ)
            s = tcp_select(soap, fd, SOAP_TCP_SELECT_RCV, soap->connect_timeout);
          else
            s = tcp_select(soap, fd, SOAP_TCP_SELECT_SND, soap->connect_timeout);
          if (s < 0)
