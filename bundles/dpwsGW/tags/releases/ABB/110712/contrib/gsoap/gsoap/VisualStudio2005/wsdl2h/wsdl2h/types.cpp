...
{ const char *name, *type, *nameURI = NULL, *typeURI = NULL;
  ...
  { char *prefix = NULL;
    ...
    { typeURI = attribute.attributePtr()->schemaPtr()->targetNamespace;
      if (!attribute.form || *attribute.form == qualified)
        nameURI = typeURI;
    }
  ...
  { fprintf(stream, "@");
    ...
    fprintf(stream, is_optional ? pointerformat : elementformat, "", aname(NULL, nameURI, name));
  }
  ...
  if (attribute.default_ || (attribute.fixed && !is_optional))
...
{ const char *name, *type, *nameURI = NULL, *typeURI = NULL;
  ...
  { char *prefix = NULL;
    ...
    { typeURI = element.elementPtr()->schemaPtr()->targetNamespace;
      if (!element.form || *element.form == qualified)
        nameURI = typeURI;
    }
  ...
  { if (!fake_union && !element.minOccurs && !element.nillable && !element.default_)
    ...
    if (element.maxOccurs && strcmp(element.maxOccurs, "1") && is_integer(element.maxOccurs))
      fprintf(stream, ":%s", element.maxOccurs);
    if (element.default_ || (element.fixed && !fake_union && (!element.minOccurs || !strcmp(element.minOccurs, "1")) && (!element.maxOccurs || !strcmp(element.maxOccurs, "1"))))
