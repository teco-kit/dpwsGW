	...
	{ Table *t;
	  ...
	  if (t)
	    ...
	    { if (!(p->info.sto & Sconst))
	      { /* replaced initializations by call to soap_default below
	        char *val = c_init(p);
	        if (p->info.hasval && *val && (p->info.typ->type != Tpointer || is_string(p->info.typ) || is_wstring(p->info.typ)))
	  ...
	  fprintf(fd," { }");
	  */
	  fprintf(fd, " { this->soap_default(NULL); }");
	}
