...
static int soap_serve_StartLDC(struct soap *soap) //TODO: pass device context
{
	...
	int op_id = OP_VLinkModel_Streaming_StartLDC;
	...
	/* prepare response */
	{
		...
		char * Action = "http://www.teco.edu/VLink/StreamingService/StartLDCOut";
...
int StreamingService_serve_request(struct soap *soap)
{
	soap_peek_element(soap);
	if (soap->action)
	{
		printf("Calling StreamingService_serve_request with soap action %s\n",soap->action);
	} else {
		printf("Calling StreamingService_serve_request without soap action\n");
	}
	if ((soap->action && !strcmp(soap->action,
			"http://www.teco.edu/VLink/StreamingService/StartLDCIn"))
