...
#define LIGHT_SENSOR_A


/**
* Switches the light sensor on
*/
#define LightSensorOn_A()	output_high(PIN_LIGHT_TSL250_POWER_A)

/**
* Switches the light sensor off
*/
#define LightSensorOff_A()	output_low(PIN_LIGHT_TSL250_POWER_A)
...
*/
#ifdef PIN_LIGHT_TSL250_OUT_A
int LightSensorInit_A()
{
	bit_clear(TRIS_LIGHT_TSL250_POWER_A);		// set power pin to output
	bit_set(TRIS_LIGHT_TSL250_OUT_A);			// set sample pin to input
	LightSensorOff_A();							// power off for light
...
* @return returns 0 if no error
*/
int LightSensorGet_A(int* value)
{
...
* @return returns 0 if no error
*/
int LightSensorPrepare_A()
{
	// set channel
	if ( !ADCIsChSet(ADCH_LIGHT_TSL250_A) ) {
		ADCSetCh(ADCH_LIGHT_TSL250_A);
	}
